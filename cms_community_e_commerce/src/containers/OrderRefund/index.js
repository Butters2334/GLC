...
@connect(
  ...
  dispatch => ({
    loadOrders: ( token, params) => {
      dispatch(getAllOrders( token, params))
    },
    updateOrderStatus: ( token, orderId, status) => {
      dispatch(updateOrderStatus( token, orderId, status))
    }
  })
)
export default class Orders extends React.Component {
  ...
  loadOrders = (params = {}) => {
    ...
    this.props.loadOrders(this.props. this.props.token, paramsData)
  }
  ...
  handleConfirmRefund = async (orderId) => {
    ...
    const res = await orderService.update( token, orderId, ORDER_REFUND_SUCCESS)
  ...
  handleRefuse = async (orderId) => {
    ...
    const res = await orderService.update( token, orderId, ORDER_REFUNDING_FAILURE)
